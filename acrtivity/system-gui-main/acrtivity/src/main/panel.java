...
if (hasError == false) {
    ...
    String rawPass = String.valueOf(password.getPassword());
    
    // 2. CHECK EMPTY FIELDS OR PLACEHOLDERS
    if(user.isEmpty() || rawPass.isEmpty() || user.equals("Enter Username...") || rawPass.equals("Enter Password")) {
        javax.swing.JOptionPane.showMessageDialog(this, "Please fill in all fields.");
        return;
    }

    try {
        // 3. CONNECT TO DATABASE (Standard Way)
        // Ensure your database name is correct (e.g., bikeshop_db)
        String url = "jdbc:mysql://localhost:3306/bikeshop_db"; 
        String dbUser = "root";
        String dbPass = "";
        ...
        // We select the role so we know which dashboard to open later
        String sql = "SELECT * FROM Tbl_user WHERE username = ? AND password = ?";
        java.sql.PreparedStatement pst = con.prepareStatement(sql);
        
        pst.setString(1, user);
        pst.setString(2, rawPass); // Note: In a real app, you should hash this!
        ...
        if (rs.next()) {
            ...
            // A. Get the role (e.g., "Admin" or "User")
            String role = rs.getString("role");
            
            // B. Close the Login Window
            this.dispose(); 
            
            // C. Open Loading Screen and tell it the Role
            // The Loading Screen will automatically open the Dashboard when it finishes!
            loading loadScreen = new loading(role);
